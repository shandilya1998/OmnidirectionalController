...
ModHopf_theta = np.empty((2,int(Tsimu_num/PhiCount))) # save Φ(t)
ModHopf_Average = np.array([[0],[X_phase/Tnum*2*pi]]) # save [Φ(t)]_t
ModHopf_X = np.empty((2,2*int(Tenum))) # save state X of last two cycle
...
for tt in tqdm(range(Tsimu_num)):
    ...
    if(tt%PhiCount==0):
        X_phase2 = Pha.Calc_phase_directory(x, ModHopf.dif, X0_, Tnum, dt, rotations = 5) # measure phase [0, Tnum-1]
    ...
    if AP == Division:
        ModHopf_Average = np.append(ModHopf_Average, np.array([[(tt-int(Tenum/2))*dt],[PhaseAverage / Division]]), axis = 1)
...
#%%
datapath = "data/ModHopf/feedback/P{}Delta{}Initial{}alpha{}/".format(P,Delta,Initial,alpha)
